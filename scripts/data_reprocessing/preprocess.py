...
# python preprocess.py --dataset_name meta.csv
...
# Load dataset
t_start = time.time()
ds = load_dataset(path=args.local_dataset_path, data_files=f'{args.dataset_name}', split="train")
print(ds)
print(f"Time to load dataset: {time.time()-t_start:.2f}")

# Run preprocessing
t_start = time.time()
ds = ds.map(preprocess, num_proc=1)
...
file_path = str(data_dir / args.dataset_name)  # Specify the path for the single output file

# Iterate over the entire dataset (no need to split into chunks)
ds_filter.to_json(file_path)  # Export the entire dataset to a JSON file

compress_file(file_path)  # Compress the file (assuming you have a function for this)
print(f"Time to save dataset: {time.time() - t_start:.2f}")
