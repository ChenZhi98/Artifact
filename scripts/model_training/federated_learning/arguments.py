...
class TrainingArguments:
    node_id: int = field(
        metadata={"help": "Node id for federated learning client."},
        default = None,  
        init = True,  # Include the field in the constructor
    )
    save_model_node_id: int = field(
        metadata={"help": "Node that save aggregated model"},
        default = 1,  # Set default value to None
    )
    ...
    base_model_dir: Optional[str] = field(
        default="../../models/initialized/gpt2", metadata={"help": "Model name or path of model to be trained."}
    )
    tokenizer_dir: Optional[str] = field(
        default="codeparrot/codeparrot", metadata={"help": "Path of tokenizer."}
    )
    save_dir: Optional[str] = field(
        default="../../models/trained/", metadata={"help": "Save dir where model repo is cloned and models updates are saved to."}
    )
    local_dataset_path: Optional[str] = field(
        default="../../dataset/google", metadata={"help": "Path to local dataset."}
    )
