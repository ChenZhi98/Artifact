...
# accelerate launch --gpu_ids='all' centralized_train.py 
...
def create_dataloaders(args):
    ...
    train_data_files = ["google_train.json.gz","microsoft_train.json.gz","facebook_train.json.gz"]

    train_datasets = []
    for train_data_file in train_data_files:
        dataset = load_dataset(path=args.local_dataset_path, data_files=f'{train_data_file}', split="train", **ds_kwargs)
        train_datasets.append(dataset)

    train_data = concatenate_datasets(train_datasets)
    train_data = train_data.shuffle(seed=args.seed)

    valid_data_files = ["google_valid.json.gz","microsoft_valid.json.gz","facebook_valid.json.gz"]

    datasets = []
    for valid_data_file in valid_data_files:
        dataset = load_dataset(path=args.local_dataset_path, data_files=f'{valid_data_file}', split="train", **ds_kwargs)
...
def log_metrics(step, epoch, metrics):
...
while current_epoch < args.max_train_epochs:
    current_epoch += 1
    for batch in train_dataloader:
        ...
        log_metrics(step, current_epoch, {"samples": step * samples_per_step, "loss_per_step/train": loss.item()})
    ...
    logger.info(f"Evaluating and saving model checkpoint for Epoch {current_epoch}")
    eval_loss, perplexity = evaluate(args)
    log_metrics(step, current_epoch, {"loss/eval": eval_loss, "perplexity": perplexity})


    # Save Model every Epoch
    accelerator.wait_for_everyone()
    save_dir = os.path.join(args.save_dir, f"epoch_{current_epoch}")
